        ...
        <div class="container-fluid">
          ...
          <div class="nav-collapse collapse">
            <p class="navbar-text pull-right" style="margin-right:5px;">
              	登录名:<a href="#" class="navbar-link"><s:property value="#session.userName"/></a>
            </p>
            <ul class="nav">
		...
		<div class="form-horizontal">
		   <legend>查询以及功能</legend>
		    	<div class="control-group">
					<button id="addSendPlan_btn" class="btn btn-primary btn-large" data-toggle="modal" data-target="#myModal">添加下发计划</button>
				</div>
		</div>
	<hr>
...
<!--				<td><s:property value="#element.verifyStatus"/></td>   -->
				...
				<td>
					<input type="button" onclick="modifyProcessStatus(<s:property value="#element.verifyStatus"/>,'<s:property value="#element.id"/>')" 
					class="btn btn-danger btn-small" value="修改状态"/>
			    </td>
			    <td style="visibility:hidden;" id="<s:property value="#element.id"/>">
			    	<input type="radio" name="<s:property value="#element.id"/>"  value="1" checked="checked"/>待执行&nbsp;&nbsp;
					<input type="radio" name="<s:property value="#element.id"/>"  value="2" />执行中&nbsp;&nbsp;
					<input type="radio" name="<s:property value="#element.id"/>"  value="3" />执行结束&nbsp;&nbsp;
					<input type="button" onclick="saveProcessStatus(<s:property value="#element.id"/>)"  class="btn btn-success btn-small" value="保存"/>
			    </td>
			</tr>
		</s:iterator>
...
<!--使用TasksController-->
	  ...
	  <script type="text/javascript">
	      $(document).ready(function(){
			...
			function modifyProcessStatus(verifyStatus, id){
//				alert(id);
				if(verifyStatus){
					alert('确定更改下发状态?');
			...
			function saveProcessStatus(id){
				var status;
				status=getRadioValue(id);		
//				alert(status);	
				window.location.href="auditAction2!modifyProcessStatus?foo.id="+id+"&foo.processStatus="+status;
			}
